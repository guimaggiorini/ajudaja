...
import { fetchAllOpportunities, fetchIBGEStates } from '../api/opportunitiesApi'
...
const OpportunitiesScreen: React.FC<OpportunitiesScreenProps> = ({ navigation }) => {
  ...
  const handleStateSelect = (state: IBGEState) => {
    ...
    Alert.alert('Região Selecionada', `Filtrando oportunidades em ${state.nome}`)
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filtersContainer}>
        ...
        >
          <TouchableOpacity 
            style={[styles.stateButton, !selectedState && styles.selectedStateButton]}
            onPress={() => setSelectedState('')}
          >
            <Text style={[styles.stateText, !selectedState && styles.selectedStateText]}>Todos</Text>
          </TouchableOpacity>
          
          {states.map((state) => (
            <TouchableOpacity 
              key={state.id}
              style={[styles.stateButton, selectedState === state.sigla && styles.selectedStateButton]}
              onPress={() => handleStateSelect(state)}
            >
              <Text 
                style={[styles.stateText, selectedState === state.sigla && styles.selectedStateText]}
              >
                {state.sigla}
              </Text>
            </TouchableOpacity>
